...
function is_login(){
    ...
    if (empty($user)) {
        ...
        return session(MODULE_NAME.'user_auth_sign') == data_auth_sign($user) ? $user['uid'] : 0;
    }
}
/**
 * 数据签名认证
...
function saveWeixinFile($filename, $filecontent)
{
	...
	$map['path']=substr($filename, 1,255);
	return $map['path'];
}
/**
 * 调用系统的API接口方法（静态方法）
...
function get_transaction_id(){
		$string=get_string(6,1);
		$map['id']=time_format(time(),'Ymd').$string;
		$thunk=M('transaction')->where($map)->getField('id');
		if($thunk){
			get_transaction_id();
		}else{
			return $map['id'];
...
function get_openid(){
	return M('AdminUser')->where(array('userid'=>is_login()))->getField('openid');
}
