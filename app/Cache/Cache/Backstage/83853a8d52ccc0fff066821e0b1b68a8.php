...
<div id="content">
  ...
  <div class="container-fluid">
        <div class="widget-box">
		  ...
		  <?php if(($_Auth) == "1"): ?><span class="btn" onclick=" window.location.href='<?php echo U('operate',array('type'=>'add'));?>'" style="padding:8px 12px;color:#333;background-color:#fff;border: 1px solid #ddd;"> <i class="icon-plus"></i> 新建会员等级</span>
				   <span class="btn ajax-get" href="<?php echo U('operate',array('type'=>'change'));?>" style="padding:8px 12px;color:#333;background-color:#fff;border: 1px solid #ddd;"> <i class="icon-refresh"></i> 切换等级条件</span><?php endif; ?>
            <table class="table table-bordered table-striped with-check">
              ...
              <tbody>			 
				...
				<?php if(is_array($_list)): foreach($_list as $key=>$vo): ?><td><input type="checkbox" class="ids" name="id[]" value="<?php echo ($vo["id"]); ?>"  /></td>
                  ...
                  <td><?php if(($vo['type']) == "1"): ?>积分<?php else: ?>现金<?php endif; ?></td>
                  <td><?php echo ($vo["auth_num"]); ?></td>
				  <td><?php echo ($vo["discount"]); ?>%</td>
				  <?php if(($_Auth) == "1"): ?><td>
					<span class="fr">	
					<?php if($vo['id'] > 2): ?><a class="btn btn-primary btn-mini" href="<?php echo U('operate',array('type'=>'edit','id'=>$vo['id']));?>">详情</a>
