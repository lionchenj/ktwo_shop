  ...
  <div class="container-fluid">
								...
								<form action="<?php echo U('index');?>" method="get" class="form-horizontal">
									...
									<div class="control-group">
										...
										<div class="controls">
											...
											<?php if(is_array($group_list)): foreach($group_list as $key=>$vo): ?><option <?php if(($vo["id"]) == $_GET['group']): ?>selected="selected"<?php endif; ?> value ="<?php echo ($vo["id"]); ?>"><?php echo ($vo["title"]); ?></option><?php endforeach; endif; ?>
											</select>
										</div>								
										</div>
        ...
        <div class="widget-box">
		  ...
		  <form name="myform" id="myform" method="post">
			...
			<span class="btn ajax-post confirm" url="<?php echo U('operate');?>" target-form="ids" style="padding:8px 12px;color:#333;background-color:#fff;border: 1px solid #ddd;"> <i class="icon-trash"></i> 批量禁用</span><?php endif; ?>
			<table class="table table-bordered table-striped with-check">
              ...
              <tbody>
				...
				<?php if(is_array($_list)): foreach($_list as $key=>$vo): ?><td align="center" valign="middle"><input type="checkbox" class="ids" name="id[]" value="<?php echo ($vo["userid"]); ?>"  /></td>
				  ...
				  <td align="center" valign="middle"><?php echo ($vo["email"]); ?></td>
				  <td align="center" valign="middle"><?php echo (get_access_groupname($vo["userid"])); ?></td>
				  <td align="center" valign="middle"><?php echo (time_format($vo["create_time"],'Y-m-d H:i:s')); ?></td>
				  <?php $group_name=get_access_groupname($vo['userid']) ?>
				  <td>
				  ...
				  <?php if(($_Auth) == "1"): ?><td>
				  <?php if(($group_name) != "医馆管理员"): ?><span class="fr">
