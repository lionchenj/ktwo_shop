...
<div id="content">
  <div id="content-header">
    <div id="breadcrumb"> <a href="/tcm/index.html" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> 首页</a> <a href="/tcm/cinema.html" class="current">中医馆列表</a> </div>
  ...
  <div class="container-fluid">
							...
							<div class="widget-content nopadding">
								<form action="<?php echo U('index');?>" method="get" class="form-horizontal">
									<div class="control-group">
										...
										<div class="controls"><input name="name" type="text" class="span3" placeholder="门店名称(模糊查找)" value="<?php echo ($_GET['username']?$_GET['username']:''); ?>"/> </div>
									</div>                         
                                    <div class="control-group">
        ...
        <div class="widget-box">
		  ...
		  <?php if(($_Auth) == "1"): ?><span class="btn" onclick=" window.location.href='<?php echo U('operate',array('type'=>'add'));?>'" style="padding:8px 12px;color:#333;background-color:#fff;border: 1px solid #ddd;"> <i class="icon-plus"></i> 新增中医馆</span><?php endif; ?>
			<table class="table table-bordered table-striped with-check">
              ...
              <tbody>
				...
				<?php if(is_array($_list)): foreach($_list as $key=>$vo): ?><td><input type="checkbox" class="ids" name="id[]" value="<?php echo ($vo["id"]); ?>"  /></td>
				  ...
				  <td><?php echo (get_group_name($vo["group"])); ?></td>
				  <td><?php echo (time_format($vo["create_time"],'Y-m-d H:i:s')); ?></td>
				  <?php if(($_Auth) == "1"): ?><td>
					<span class="fr">
					<a class="btn btn-warning btn-mini" href="<?php echo U('exmember/index',array('cinema'=>$vo['id']));?>">核销人员</a>
					<a class="btn btn-primary btn-mini" href="<?php echo U('operate',array('type'=>'edit','id'=>$vo['id']));?>">详情</a>
					<a class="btn btn-danger btn-mini confirm ajax-get" href="<?php echo U('operate',array('type'=>'del','id'=>$vo['id']));?>">删除中医馆</a>
					</span>
					</td><?php endif; ?>
