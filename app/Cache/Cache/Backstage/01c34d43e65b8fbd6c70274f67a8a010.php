...
<div id="content">
  <div id="content-header">
    <div id="breadcrumb"> <a href="/tcm/index.html" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> 首页</a> <a href="/tcm/group.html" class="current">中医馆集团列表</a> </div>
	<h1>新增中医馆集团</h1>
  </div>
					...
					<div class="span12">
						<div class="widget-box">
							...
							<div class="widget-content nopadding">
								<form action="/tcm/Group/operate/type/add.html" method="post" class="myform form-horizontal">
									<div class="control-group">
										<label class="control-label">集团名称:</label>
										<div class="controls">
											<input  type="text" name="name" class="span3" placeholder="集团名称" value=""/>
											<div id="error_name" ><error class="color-red hidden"></error></div>
										</div>
									</div>
									
									<div class="form-actions">
										<button id="submit" type="submit" class="btn btn-success">添加集团</button>
											<button type="button" class="btn btn-warning"  onclick="window.location.href='/tcm/group.html'">返回</button>
									</div>
								</form>
							</div>
						</div>						
...
$("input").blur(function(){
  ...
  switch(name)
  {
	...
	if(vlaue.length==0){
			...
			$(this).siblings('div').find('error').html("<span>*</span>请输入集团名称");
...
$(".myform").submit(function(){
	...
	if(name.length==0){
		...
		$("input[name='name']").siblings('div').find('error').html("<span>*</span>请输入集团名称");
