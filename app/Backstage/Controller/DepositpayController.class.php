...
class DepositpayController extends PublicController {
		...
		protected function pay(){
			...
			$card=M('reg_card')->where(array('status'=>1))->select();
			$this->assign('card',$card);
			$this->display('deposit_pay');
		}
		protected function pay_ajax(){
			...
			$card_id=I('card_id');
			$card_id 	|| 	$this->error('请选择充值卡');
			$card=M('reg_card')->where(array('id'=>$card_id,'status'=>1))->find();
			$user=M('member')->where(array('mobile'=>$mobile))->find();
			$user  || $this->error('用户不存在');
			$card  || $this->error('充值卡不存在');
			$map['mobile']=$mobile;
			$map['status']=1;
