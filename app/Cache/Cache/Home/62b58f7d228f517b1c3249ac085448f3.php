...
<body>
	<div class="new-container">
		...
		<div class="form-wrap relative">
			...
			<a href="<?php echo U('distribution',array('orderid'=>$data['orderid']));?>" class="record-link abs">查看分劵记录</a>
		</div>
	</div>
	...
	<script type="text/javascript">
		...
		$(document).on('click','.submit-btn',function(){
			...
			if(num<1){
				...
				var url="/shadow/mast/Home/About/edit/orderid/x20171019140444.html";
				var query = {};
				query['orderid']='<?php echo ($data["orderid"]); ?>';
				query['num']=num;
				$.post(url,query).success(function(res){
					if(res.status==1){
						var url2="<?php echo U('receive',array('id'=>abc));?>";
