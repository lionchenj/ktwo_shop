...
<div id="content">
  <div id="content-header">
    <div id="breadcrumb"> <a href="/tcm/index.html" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> 首页</a> <a href="/tcm/exmember.html" class="current">中医馆集团列表</a> </div>
	<h1>新增核销人员</h1>
  </div>
					...
					<div class="span12">
						<div class="widget-box">
							...
							<div class="widget-content nopadding">
								<form action="/tcm/Exmember/operate/type/add.html" method="post" class="myform form-horizontal">
									<div class="control-group">
										<label class="control-label">中医馆名称:</label>
										<div class="controls">
											<select name="cinema" id="cinema">
											<option value ="0">请选择</option>
											<?php if(is_array($cinema)): foreach($cinema as $key=>$_cinema): ?><option  value ="<?php echo ($_cinema["id"]); ?>"><?php echo ($_cinema["name"]); ?></option><?php endforeach; endif; ?>
										</select>
										</div>
									</div>
									<div class="control-group">
										...
										<div class="controls">
											...
											<option value ="0">请选择</option>
											<?php if(is_array($member)): foreach($member as $key=>$_member): ?><option  value ="<?php echo ($_member["openid"]); ?>"><?php echo ($_member["nickname"]); ?></option><?php endforeach; endif; ?>
										</select>
										</div>
									</div>
									...
									<div class="form-actions">
										<button id="submit" type="submit" class="btn btn-success">添加核销人员</button>
											<button type="button" class="btn btn-warning"  onclick="window.location.href='/tcm/exmember.html'">返回</button>
									</div>
								</form>
							</div>
						</div>						
