...
class PublicController extends Controller {
    ...
    protected function _initialize(){
		...
		// $wechatConfig=array(
		// 'appId'=>C('WECHAT_APPID')?C('WECHAT_APPID'):'wx6c4304cc9167e16c',
		// 'appSecret'=>C('WECHAT_APPSECRET')?C('WECHAT_APPSECRET'):'43ce39e82ed0b997e5480215e68a3658',
		// );
		// $model = new \Wechat($wechatConfig);
		// $res=$model->getUserInfo($_GET['code']);
		// if($res['status']==1){
		// 	$user_update['openid']=$res['userInfo']['openid'];
		// 	M('admin_user')->where(array('userid'=>is_login()))->setField($user_update);
		// }else{
     ...
     * @author shadow  <m89520@163.com>
     */
    final public function getMenus($url='',$level=1){
    	empty($url)?$url=CONTROLLER_NAME.'/'.ACTION_NAME:$url;
        if(empty($menus)){
			$where['hide']	=	0;
