...
class DepositcardController extends PublicController {
		...
		protected function pay_ajax(){
			...
			if($info['create_time']+300>=time()){
				...
				$card_info=M('reg_code')->where(array('code'=>$card,'status'=>0))->find();
				if(!$card_info){
					$this->error('充值卡不存在或已使用');
				}			
				$order['orderid']=get_charge_orderid_chang();
				$order['goodsid']=$card_info['goodsid'];
				$order['money']=$card_info['total'];
				...
				M('reg_code')->where(array('code'=>$card,'status'=>0))->setField('status',1);//注销充值卡	
				$ll_order['no_order']=get_payid_chang();
				$ll_order['orderid_list']=$order['orderid'];
				$ll_order['oid_partner']='充值卡充值';
				$ll_order['money']=0;
