...
<div id="content">
  <div id="content-header">
	...
	<h1><?php if(!empty($card)): ?>修改充值卡<?php else: ?>新增充值卡<?php endif; ?></h1>
  </div>
					...
					<div class="span12">
						<div class="widget-box">
							<div class="widget-title">
								...
								<h5><?php if(!empty($card)): ?>修改充值卡<?php else: ?>新增充值卡<?php endif; ?></h5>
							</div>
							<div class="widget-content nopadding">
								<form action="/tcm/Regcard/operate/type/add.html" method="post" class="myform form-horizontal">
									<div class="control-group">
										<label class="control-label">充值金额:</label>
										<div class="controls">
											<input  type="text" name="money" class="span2" placeholder="充值金额" value="<?php echo ($card["money"]); ?>"/>元
									...
									<div class="control-group">
										...
										<div class="controls">
											<input  type="text" name="give" class="span2" placeholder="赠送金额" value="<?php echo ($card["give"]); ?>"/>元
											<div id="error_name" ><error class="color-red hidden"></error></div>
										</div>
									</div>
									<div class="form-actions">
										<button id="submit" type="submit" class="btn btn-success"><?php if(!empty($card)): ?>保存<?php else: ?>创建<?php endif; ?></button>
											<button type="button" class="btn btn-warning"  onclick="window.location.href='/tcm/regcard.html'">返回</button>
									</div>
								</form>
							</div>
						</div>						
		...
		$.post(url,query).success(function(data){
				 if (data.status==1) {
					 $('#popbox .controls').html('保存成功');
						center($("#popbox"));
						 setTimeout(function(){
						 <?php if(!empty($card)): ?>location.href=url;
						 <?php else: ?>
						location.href='/tcm/payorder.html';<?php endif; ?>
                    },1500);
				 }else{
